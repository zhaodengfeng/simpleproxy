...
validate_domain() {
    ...
    # Allows: alphanumeric, hyphens, dots; must start and end with alphanumeric
    if [[ -z "$domain" ]]; then
        return 1
    fi
    # Check length (max 253 chars for FQDN, max 63 per label)
    if [[ ${#domain} -gt 253 ]]; then
        return 1
    fi
    # Validate domain format: labels separated by dots, each label max 63 chars
...
# Security: Validate password/key (alphanumeric + common special chars, no shell metacharacters)
validate_key() {
    local key="$1"
    local max_len="${2:-256}"
    if [[ -z "$key" ]]; then
    ...
    # Allow alphanumeric, base64 chars (+, /, =), and common safe special chars
...
# Security: Generate secure random values using openssl or /dev/urandom
gen_random() {
    local length="${1:-16}"
    local result=""
    ...
    # Prefer openssl for cryptographically secure random
    if command -v openssl &>/dev/null; then
        result=$(openssl rand -base64 64 2>/dev/null | tr -dc 'a-zA-Z0-9' | head -c "$length")
    fi

    # Fallback to /dev/urandom if openssl fails or not available
    if [[ -z "$result" ]]; then
    ...
    if [[ -z "$result" ]]; then
        echo "ERROR: Failed to generate secure random value" >&2
        return 1
    fi
